...
import RadioGroupDefaultWrapper from './form-elements/RadioGroupWrapper';
// validation schema
...
const RequestForm = () => {
	const [currentStep, setCurrentStep] = useState('biodata');
	...
	const schema = yup.object({
		...
		sexAtBirth: yup.string().required('Please provide your sex at birth'),
		...
		thirdPartyEmail: yup
			...
			.when('$thirdParty', {
				is: true,
				then: yup.string().required('Please provide email'),
			}),
		thirdPartyPhone: yup.string().when('$thirdParty', {
			is: true,
			then: yup.string().required('Please provide phone number'),
		}),
			...
			<div className="mb-12 w-full">
				<div className="flex flex-wrap w-full">
                    ...
                    `}
							...
							<button
								type="button"
								onClick={() => setCurrentStep('bicycle')}
								className={`
                    text-small
						...
						<>
							...
							<div className="p-4 bg-primary-container/30 rounded-lg shadow w-full">
								<p className="text-sm my-6 italic">
									...
									<span className="text-error">*</span> are
									required.
								</p>
					...
					{currentStep === 'bicycle' && (
						<>
							<div className="w-full max-w-full text-center flex flex-col mb-12 gap-2">
								<h5 className="text-h5 font-bold">
									Tell us about the bicycle that will be a
									good fit for you
								</h5>
								<h6 className=" text-h6 text-on-surface-variant">
									Please provide us with bicycle fitting
									information
								</h6>
							</div>
							<div className="p-4 bg-primary-container/30 rounded-lg shadow w-full">
									...
									<div className="flex flex-col">
										<div className="space-y-3">
											<label className="text-paragraph font-medium relative">
												Where would you store your bike?{' '}
												<span className="text-error px-1">
													*
												</span>
												<span className="block text-extraSmall italic">
